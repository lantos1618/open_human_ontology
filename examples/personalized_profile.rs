...
use human_biology::biology::genetics::{AceGenotype, Actn3Genotype};
...
fn main() {
    ...
    let mut profile =
        ComprehensiveHealthProfile::new("EXAMPLE_001".to_string(), 28, BiologicalSex::Male);
    ...
    profile.genetics.dermatology_risks.acne = Some(AcneRisk::new(false, false, false, false));

    profile.genetics.dermatology_risks.psoriasis =
        Some(PsoriasisRisk::new(false, false, false, false, false));
    ...
    let taste = TasteReceptorGenetics::new(Tas2r38Genotype::PavAvi, true, true);
    ...
    if let Some(ref skin_genetics) = profile.genetics.skin_genetics {
        ...
        println!(
            "  Sun Protection Needed (UV index 6): {}",
            skin_genetics.sun_protection_needed(6.0)
        );
        println!(
            "  Recommended Sun Exposure: {:.0} minutes\n",
        ...
        .predicted_fiber_type_ratio
    {
        ...
        println!(
            "  Endurance Oriented: {}",
            fiber_ratio.is_endurance_oriented()
        );
    ...
    println!(
        ...
        measurements.waist_to_height_ratio()
    );

    let composition = &profile.anthropometry.composition;
    println!("  Body Fat: {:.1}%", composition.body_fat_percentage());
    println!(
        "  Fat-Free Mass Index: {:.1}",
        composition.fat_free_mass_index(measurements.height_cm)
    );
