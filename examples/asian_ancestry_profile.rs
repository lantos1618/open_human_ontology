...
use human_biology::biology::genetics::{Actn3Genotype, AceGenotype, AncestryProfile, AncestryPopulation};
...
fn main() {
    ...
    if let Some(ref skin_genetics) = profile.genetics.skin_genetics {
        ...
        println!("  Melanoma Baseline Risk: {:.2}",
            skin_genetics.fitzpatrick_type.melanoma_baseline_risk());
        println!("  Vitamin D Synthesis Rate: {:.2}x",
    ...
    if let Some(ref hair_genetics) = profile.genetics.hair_genetics {
        ...
        println!("  Texture: {:?} (EDAR 370A variant)", hair_genetics.predicted_texture);
        println!("  Thickness: {:.0} micrometers (thicker, typical for East Asian)",
    ...
    if let Some(ref chemosensory) = profile.genetics.chemosensory {
        ...
        println!("  Bitter Sensitivity: {:.2}x (Non-taster - AVI/AVI genotype)", taste_profile.bitter);
    ...
    println!("  East Asian BMI Threshold: {:.1} (lower than standard 25.0)",
    ...
    if let Some(ref chol) = profile.health_metrics.cholesterol {
        ...
        println!("  HDL: {:.0} mg/dL (Good - typical for East Asian women)", chol.hdl);
