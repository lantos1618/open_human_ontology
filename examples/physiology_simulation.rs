...
fn main() {
    ...
    println!("\n=== Simulation Summary ===");
    println!("Total simulation time: {:.1} minutes", sim.time.elapsed_minutes());
    println!("Total iterations: {}", sim.time.iteration);
    println!("History points: {}", sim.history.len());

    println!("\n--- Health Score Progression ---");
    let checkpoints = [0, sim.history.len() / 4, sim.history.len() / 2,
                       3 * sim.history.len() / 4, sim.history.len() - 1];
    for &idx in &checkpoints {
        let (time, state) = &sim.history[idx];
        println!("  {:.1} min: {:.1}%", time / 60.0, state.overall_health_score() * 100.0);
    }
...
fn print_state(state: &PhysiologyState, time_min: f64) {
    ...
    println!("  BP: {:.0}/{:.0} mmHg",
             state.cardiovascular.systolic_bp_mmhg,
             state.cardiovascular.diastolic_bp_mmhg);
    println!("  CO: {:.2} L/min", state.cardiovascular.cardiac_output_l_min);
    println!("  SVR: {:.0} dynes⋅s/cm⁵", state.cardiovascular.systemic_vascular_resistance);

    println!("\nRespiratory:");
    println!("  RR: {:.1} /min", state.respiratory.respiratory_rate_per_min);
    println!("  TV: {:.0} mL", state.respiratory.tidal_volume_ml);
    println!("  MV: {:.2} L/min", state.respiratory.minute_ventilation_l_min);
    ...
    println!("  Glucose: {:.1} mg/dL", state.metabolic.blood_glucose_mg_dl);
    println!("  Lactate: {:.2} mmol/L", state.metabolic.blood_lactate_mmol_l);
    println!("  MR: {:.0} kcal/day", state.metabolic.metabolic_rate_kcal_day);

    println!("\nNeurological:");
    println!("  Sympathetic: {:.2}", state.neurological.sympathetic_tone);
    println!("  Parasympathetic: {:.2}", state.neurological.parasympathetic_tone);
    println!("  Catecholamines: {:.1} ng/mL", state.neurological.catecholamine_level_ng_ml);
    println!("  Cortisol: {:.1} µg/dL", state.neurological.cortisol_level_ug_dl);

    println!("\nRenal:");
    println!("  GFR: {:.1} mL/min", state.renal.gfr_ml_min);
    println!("  Urine output: {:.1} mL/hr", state.renal.urine_output_ml_hr);
    ...
    println!("Overall Health Score: {:.1}%", state.overall_health_score() * 100.0);
}
...
fn analyze_simulation(sim: &PhysiologySimulation) {
    ...
    let co_change = last.cardiovascular.cardiac_output_l_min - first.cardiovascular.cardiac_output_l_min;
    println!("   HR change: {:+.1} bpm", hr_change);
    println!("   CO change: {:+.2} L/min", co_change);

    println!("\n2. Respiratory Response:");
    let rr_change = last.respiratory.respiratory_rate_per_min - first.respiratory.respiratory_rate_per_min;
    ...
    let para_change = last.neurological.parasympathetic_tone - first.neurological.parasympathetic_tone;
